...
public:
	output_algorithm(std::vector<Point> _final_centers, std::vector<int> _final_labels, double _cost, int _iterations) :
		final_centers{_final_centers}, final_labels{_final_labels}, cost{_cost}, iterations{_iterations} {}
...
public:
   ...
   /// </summary>
	...
	{ 
		// find corresponding delimeter in file
		delimiter = get_delimiter(filepath);
		points = read_file(filepath, delimiter); 
	}
...
public:
	...
	information_clustering(std::vector<Point>& _centers, std::vector<double>& _closest_center_distances, std::vector<int>& _labels, 
		std::vector<double>& _second_closest_center_distances, std::vector<int>& _second_closest_labels, std::vector<double>& _cumsum) :
		information_clustering(_closest_center_distances, _labels, _second_closest_center_distances, _second_closest_labels, _cumsum) {
		centers = _centers;
	}

	information_clustering(std::vector<double>& _closest_center_distances, std::vector<int>& _labels,
		std::vector<double>& _second_closest_center_distances, std::vector<int>& _second_closest_labels, std::vector<double>& _cumsum) :
		closest_center_distances(_closest_center_distances), second_closest_center_distances(_second_closest_center_distances),
	...
	void set_next_cumsum_value(int index, double value) {
		if (index == 0) cumsum[index] = value;
		else cumsum[index] = cumsum[index - 1] + value;
	}
};
