...
bool Clustering_Algorithm::update_labels() {
	...
	for (int i = 0; i < points.size(); ++i) {
		...
		if (!change && labels[i] != current_min_label) change = true;
...
int Clustering_Algorithm::choose_center()
{
	...
	if (cumsums.back() == 0) {	// cost can not be reduced, return last point
...
output_algorithm KMEANS::algorithm(int k, bool init, double _old_cost) {
	...
	while (change) {
		...
		if (iterations == maximum_number_iterations) break;
...
void KMEANS::initialize_centers(int k) {
	...
	if (centers.size() > 0) centers.resize(0);
...
{	
	...
	for (int i = 0; i < points.size(); i++) {
		...
		if (new_closest[i]) { // if new_closest[i] = true, the added_center becomes closest and old closest becomes secondclosest
		...
		else { // otherwise we check if added_center becomes new second_closest
...
bool KMEANS::check_break_cond(int iterations, double old_cost, double new_cost, std::vector<Point> old_centers, std::vector<Point> centers)
{
	...
	if (iterations == 0 || old_cost ==-1) return false;

	// old variant for break condituen
	if (1 - new_cost / old_cost < tol_factor) return true;
...
bool KMEANS::update_labels() {
	...
	for (int i = 0; i < points.size(); ++i) {
		...
		if (!change && current_min_dist_label != labels[i]) change = true;
...
void GREEDY_KMEANS::initialize_centers(int k)
{
	...
	// first select the first center uniformly at random
	for (int i = 1; i < z; i++) {
		//int new_rand = ((int)(rand() * points.size() / RAND_MAX));		// TODO: use random generator
	...
	//labels = best_information.labels;		// not necesarry since only one center exists so all points have the same label as before
	closest_center_distances = best_information.closest_center_distances;
	//second_closest_center_distances = best_information.second_closest_center_distances;	// since only one center exists, the distance to the secondclosest (infty) remains the same
	second_closest_labels = best_information.second_closest_labels;
	cumsums = best_information.cumsum;	

	//update_distances(); // labels stay the same but the currently saved distances might have changed
	brute_force_labels_compare();
...
void LOCAL_SEARCH::greedy_local_search_center() {
	...
	for (int i = 0; i < centers.size(); ++i) {
		...
		for (int j = 0; j < points.size(); j++) {
			...
			if (labels[j] != i) {	// assigned center of point j is not the removed center => compare with closest distance
			...
			else {	// assigned center of point j was removed => we compare with second closest distance
		...
		if (new_cost < best_cost) {	// exchange did make improvement
			best_cost = new_cost;
		...
		if (found_improvement) {
			...
			for (int j = 0; j < points.size(); j++) {				
				...
				if (best_candidate_closer[j]) { // we already know, if this is set to true, that the new candidate becomes the best center (or second closest)
					if (labels[j] == best_exchange) { // closest center was replaced with candidate, candidate is new closest center, secondclosest center stays the same
						closest_center_distances[j] = best_candidate_distances[j];
						// label does not change, secondclosest remains unchanged
					}
					else {	// we did not exchange the old closest center => old closest center becomes second closest and candidate is new closest center
				...
				else {	// the candidate was not better than the closest (or secondclosest if closest center removed) center => we check if it can become secondclosest
					if (labels[j] == best_exchange) {	// closest center was removed and candidate is not new closest => second closest becomes closest and we need to compute thirdclosest
						...
						if (third_closest_distance < best_candidate_distances[j]) {	// the thirdclosest is closer than the canidate
						...
						else {	// candidate becomes new secondclosest
							second_closest_center_distances[j] = best_candidate_distances[j];
							second_closest_labels[j] = best_exchange;
						}
					}
					else {	// closest center was not removed and candidate was not closer than old closest center => compare with second closest
						if (second_closest_labels[j] != best_exchange) {	// the second closest center is not the same as the exchanged center => compare candidate with secondclosest for possible update
							if (second_closest_center_distances[j] > best_candidate_distances[j]) {
								second_closest_center_distances[j] = best_candidate_distances[j];
								second_closest_labels[j] = best_exchange;
							}
							else { // since neither closest nor secondclosest were removed and candidate is further away we do nothing
							}
						}
						else {	// the secondclosest was removed => we need to find the third closest for comparison
							std::pair<std::vector<int>, std::vector<double>> third_closest_info = find_3_closest(j);
...
std::pair<std::vector<int>, std::vector<double>> LOCAL_SEARCH::find_3_closest(int point)
{
	...
	for (int i = 0; i < centers.size(); i++) {
		...
		if (current_distance < closest_distance) { // update all distances
		...
		else if (current_distance < second_closest_distance) {	// closest still closest but second closest gets updated
			third_closest = second_closest;
			third_closest_distance = second_closest_distance;
		...
		else if (current_distance < third_closest_distance) {	// only thirdclosest gets updated
...
void FLSPP::local_search_foresight_iterations(int iterations_foresight)
{
	...
	for (int it = 0; it < iterations_foresight; it++)
	{
		...
		*/
		...
		int i;
		for (int i = 0; i < centers.size(); i++) { // since we always do these number of iterations we do not care about the order of exchanges
			for (int j = 0; j < points.size(); j++) {
				if (labels[j] != i) { // center was not exchanged
			...
			// compute new clustering after one step using newly computed labels in previous step
			//change_new = single_loyds_step(new_labels, centroids_new, closest_center_distances_changed, closest_center_distances_changed_labels,
				//second_closest_center_distances_changed, second_closest_center_distances_changed_labels, new_cumsums, new_clustercosts);
			...
			//if (current_min_cost > new_cumsums.back()) {
			if (current_min_cost > new_cost){
				/*centroids_best = centroids_new;
				closest_center_distances_best = closest_center_distances_changed;
				closest_center_distances_labels_best = closest_center_distances_changed_labels;
				second_closest_center_distances_best = second_closest_center_distances_changed;
				second_closest_center_distances_labels_best = second_closest_center_distances_changed_labels;
				new_cumsums_best = new_cumsums;
				change_best = change_new;

				current_min_cost = new_cumsums.back();*/
...
// #################### BACKUP OF OLD VERSION OF FLSPP #####################################
...
//output_algorithm FLSPP::algorithm(int k, bool init, double _old_cost) {
//
//	// Initialize configuration using number of centers
//	FLSPP_configuration new_conf(k);
//	my_conf = new_conf;
//
//	// First we use some standard initialization method (could be made more generally as an additional parameter)
//	if (init) initialize_centers(k);
...
//	set_clustercosts();
//
//	// if we use default strategy we compute the order to iterate through the centers for exchange in the same manner every time
//	if (my_conf.default_order()) my_conf.set_default_order();
//	else if (my_conf.decreasing_clustercosts) my_conf.set_decreasing_clustercosts_order(clustercosts);
//
//	bool change = true;
//
//	double new_cost;
...
//
//
//	//local_search_foresight_iterations(max_number_iterations_foresight); 		// if max_number_iterations_fls is set to -1 we skip this	
//
//
//	bool stop_by_factor = false;
//	bool stop_by_iterations = false;
//	bool found_improvement = false;
//
//
//	while (change) {
//
//		found_improvement = false;	// bool for checking if some exchange made an improvement
//
//		if (iterations == maximum_number_iterations) {		// this value is set to -1 in default
//			stop_by_iterations = true;
//			break;
//		}
//
//		old_centers = centers;
...
//		}
//
//		// compute cost of one loyds step using same set of centers
//		change_best = single_loyds_step(labels, centroids_best, closest_center_distances_best, closest_center_distances_labels_best,
//			second_closest_center_distances_best, second_closest_center_distances_labels_best, new_cumsums_best, new_clustercosts_best);
//
//		double current_min_cost = new_cumsums_best.back();
...
//
//		//for (int i = 0; i < centers.size(); i++) {
//		// we use self defined ordering
//		int i;
//		for (int l = 0; l < my_conf.exchange_center_order.size(); l++) {
//			i = my_conf.exchange_center_order[l];
//			for (int j = 0; j < points.size(); j++) {
//				if (labels[j] != i) { // center was not exchanged
//					if (closest_center_distances[j] <= new_distances[j]) {
//						new_labels[j] = labels[j];
...
//				change_best = change_new;
//
//				// update old centers accordingly
//				old_centers = centers;
//				old_centers[i] = points[center_candidate];
//
//				current_min_cost = new_cumsums.back();
//
//				// update improvement variable
//				found_improvement = true;
//
//				// If first_improve is set to true we end the iterations
//				if (my_conf.first_improve) break;
//			}
//		}
//
//		// Update information (this could be modified to assign pointers)
...
//		change = change_best;
//
//		// If no improvement was found we increase the counter of failed iterations, otherwise we reset to 0
//		if (!found_improvement) my_conf.update_fail_condition();
//		else my_conf.reset_fail_condition();
//
//		// check for correctness (if makro is defined)
//		brute_force_labels_compare();
//
//		// update new cost and check if stop condition factor is reached
//		new_cost = cumsums.back();
//		/*if (iterations != 0 && 1 - new_cost / old_cost < tol_factor) {
//			stop_by_factor = true;
//			break;
//		}*/
//
...
//		iterations++;
//
//		if (my_conf.check_fail_condition()) break;
//	}
//
//	// check if we have terminated because number of failed consecutive iterations was reached
//	// If this was the case we continue with standard implementation
//	if (my_conf.check_fail_condition()) return GREEDY_KMEANS::algorithm(k, false, new_cost);
//
//	return output_algorithm(centers, labels, cumsums.back(), iterations);
//}